...
const TesterCommand: Command = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
            subcommand
                .setName('give')
                .setDescription('Accorde le statut Testeur à un utilisateur. (Propriétaire du bot seulement)')
                ...
                .addStringOption(option =>
                    option.setName('duration')
                        .setDescription('La durée du statut (ex: 30d, 1m, 1y). Laisser vide pour permanent.')
                        .setRequired(false)))
        .addSubcommand(subcommand =>
            subcommand
                .setName('revoke')
                .setDescription('Révoque le statut Testeur d\'un utilisateur. (Propriétaire du bot seulement)')
                .addUserOption(option =>
                    option.setName('user')
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (subcommand === 'give' || subcommand === 'revoke') {
            ...
            // This part is usable by server admins
