...
const SlotsCommand: Command = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (userLevel.xp < betAmount) {
            await interaction.reply({ content: `Vous n'avez pas assez d'XP pour miser ce montant. Vous avez actuellement ${userLevel.xp} XP.`, ephemeral: true });
            return;
        }
        ...
        const embed = new EmbedBuilder()
            ...
            .setFooter({ text: `Nouveau solde : ${userLevel.xp + winnings} XP`})
            .setColor(color);
