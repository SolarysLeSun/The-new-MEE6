...
const RappelCommand: Command = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (delayMs > ms('30d')) {
            await interaction.reply({ content: 'Le délai de rappel не peut pas dépasser 30 jours.', ephemeral: true });
            return;
        }
        ...
        setTimeout(async () => {
            const embed = new EmbedBuilder()
                .setColor(0x3498DB)
                .setTitle('⏰ Rappel')
                .setDescription(`Vous m'avez demandé de vous rappeler ceci <t:${creationTimestamp}:R> :\n\n> ${message}`)
