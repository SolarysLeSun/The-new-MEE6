...
const LockCommand: Command = {
    data: new SlashCommandBuilder()
        .setName('lock')
        .setDescription('Verrouille un salon, empêchant les membres de parler.')
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        try {
            ...
            const rolesToLock = config.roles_to_lock || [];
            rolesToLock.push(interaction.guild.id); // Always add @everyone

            const permissionUpdates = rolesToLock.map(roleId => {
